...
<br>
      <div class="price">
        <h1 style="font-size: 20px;">Price range</h1>
        <input type="number" name="min" style="width: 80px; height: 35px;border-radius: 8px;" > -
        <input type="number" name="max" style="width: 80px; height: 35px;border-radius: 8px;">
        <input type="submit" name="sbtn" value="Go" style="width: 50px; height: 35px; position: relative;left: 5%; border-radius: 10px;" onclick="price_check(min,max)">
...
//$query = "SELECT T.*, COUNT(C.id) as count FROM tbl_images T, checkout C ORDER BY p_cost desc" ;  
                      $query = "SELECT * FROM tbl_images  where p_cost BETWEEN '0' AND '50000'" ;  
                      $result = mysqli_query($connect, $query); 
...
<br><br><br>
                  ...
                  else
                  {
                    ...
                    echo '<table ><tr>
                        ...
                        <center><div class="displaypage" >
                          <th><a  href="detail_display.php?id='.$row['id'].'" style="color:#fff;position:relative; left:-6%;"><img src="data:image/jpeg;base64,'.base64_encode($row['name'] ).'" height="300" width="300" class="img-thumnail" style="margin-right:100px; margin-bottom:50px;" /> </a>
